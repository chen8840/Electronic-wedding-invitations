    ...
    <script>
        $(function(){
            ...
            function loadPage(index, size) {
                size = size || 10;
                $.post('{!! url('fetchmessages') !!}/' + index + '/' + size, function(data) {
            ...
            function setRead(id) {
                ...
                if($parent.hasClass('panel-success')) {
                    ...
                    $.post('{!! url('setreadmessage') !!}/' + id, function(data) {
...
@section('content')
    <div class="row">
        <div class="alert alert-info">
            你有<span id="no_read_count">{!! count(\App\Classes\Logic\Message::getNoReadMessages4User($U->id)) !!}</span>条未读消息。
        </div>
    </div>
    <div class="row">
